...
export default function Marcas() {
 ...
 const [loading, setLoading] = useState(false)
  ...
  const guardarMarca = async () => {
    ...
    setLoading(true)
    try{
      const res = await crearMarca(marcas)
      console.log(res)
      setLoading(true)
      setMarca({nombre: ''})
      listMarcas()
  ...
  const editarMarca = async (e) => {
    ...
    try{
      setError(false)
      const resp = await editarMarcaPorID(marcas._id, marcas);
      console.log(resp)
      resetMarca()
      listMarcas()
    }catch(e){
              ...
              <div className="modal-body">
                <form onSubmit={editarMarca}>
                  ...
                  <button 
                    ...
                    disabled={marcas.nombre.length <= 0}
                    data-bs-dismiss="modal"
                  >
                    Enviar
                  </button>
        ...
        <tbody>
          {
            marcas.map((marcas,index) => {
              ...
              <tr key={marcas._id}>
                ...
                <td>{marcas.nombre}</td>
                <td>{marcas.estado ? 'Activo': 'Inactivo'}</td>
                <td>{dayjs(marcas.fechaCreacion).format('YYYY-MM-DD')}</td>
                <td>{dayjs(marcas.fechaActualizacion).format('YYYY-MM-DD')}</td>
                <td>
                  <button 
                    id={marcas._id}
                    type="button" 
